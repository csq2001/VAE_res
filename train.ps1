...
$env:VAE_STAGE1_EPOCHS = if ($env:VAE_STAGE1_EPOCHS) { $env:VAE_STAGE1_EPOCHS } else { "0" }
$env:VAE_STAGE2_EPOCHS = if ($env:VAE_STAGE2_EPOCHS) { $env:VAE_STAGE2_EPOCHS } else { "5" }
$env:VAE_STAGE3_EPOCHS = if ($env:VAE_STAGE3_EPOCHS) { $env:VAE_STAGE3_EPOCHS } else { "0" }
$env:VAE_STAGE3_LR_FACTOR = if ($env:VAE_STAGE3_LR_FACTOR) { $env:VAE_STAGE3_LR_FACTOR } else { "0.05" }
...
$env:VAE_CHECKERBOARD_CONTEXT = if ($env:VAE_CHECKERBOARD_CONTEXT) { $env:VAE_CHECKERBOARD_CONTEXT } else { "1" }
...
$env:VAE_RESUME_STAGE2 = if ($env:VAE_RESUME_STAGE2) { $env:VAE_RESUME_STAGE2 } else { "outputs/checkpoints/best.pth" }
$env:VAE_RESUME_STAGE3 = if ($env:VAE_RESUME_STAGE3) { $env:VAE_RESUME_STAGE3 } else { "outputs/checkpoints/best.pth" }
